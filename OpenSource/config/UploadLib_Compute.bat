...
if "%build_type%"=="Debug" (
		...
		rem ///上传到本地
		xcopy "Debug\%libName%"    "%DirLocal%\Compute\Lib" /y /d /f
) else (

		echo *************拷贝Release 库文件 至本地及服务器****
	  	rem ///上传到本地
		xcopy "Release_%pf%\%libName%"    "%DirLocal%\Compute\Lib" /y /d /f
		
		rem ///上传到远程
		xcopy "Release_%pf%\%libName%"    "%DirServer%\Compute\Lib" /y /d /f	
...
if "%build_type%"=="Debug" (
		...
		rem ///上传到本地
		xcopy "Debug\%arxName%"    "%DirLocal%\Compute\dll" /y /d /f
		xcopy "Debug\%arxName%"    "%DirExport%\ddll\sys\win32" /y /d /f
) else (

		echo *************拷贝Release 执行文件 至本地及服务器****
	  	rem ///上传到本地
		xcopy "Release_%pf%\%arxName%"    "%DirLocal%\Compute\dll" /y /d /f
