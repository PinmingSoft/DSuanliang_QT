...
// PmArray<T,R>, of objects of type "T".
//
...
//
// In order to use the class PmArray<T,R>, you need to understand
...
// should use the PmArrayMemCopyReallocator template with the array.
// If the class you intend to contain requires the use of operator=() for
// the copying during reallocation you should use PmArrayObjectCopyReallocator
//
// The default behaviour is to use the PmArrayMemCopyReallocator
//
template <class T> class PmArrayMemCopyReallocator
{
public:
...
template <class T> class PmArrayObjectCopyReallocator
{
public:
	static void reallocateArray(T* pCopy, const T * pSource, int nCount)
	{
...
template <class T, class R = PmArrayMemCopyReallocator<T> > class PmArray
{
public:
	PmArray(int initPhysicalLength = 0, int initGrowLength = 8);
	PmArray(const PmArray<T,R>&);
	~PmArray();

	// Copy operator.
	//
	PmArray<T,R>&         operator =  (const PmArray<T,R>&);
	bool                operator == (const PmArray<T,R>&) const;
	...
	T &             at          (int index);
	PmArray<T,R>&         setAt       (int index, const T& value);
	PmArray<T,R>&         setAll      (const T& value);
	T&                  first       ();
	const T &           first       () const;
	T&                  last        ();
	const T &           last        () const;
	...
	PmArray<T,R>&         append      (const PmArray<T,R>& array);
	PmArray<T,R>&         insertAt    (int index, const T& value);

	/// 添加value到数组，如果数组中已经有value，则不添加直接返回该值所在的位置
	int  appendDifferent(const T& value);
	PmArray<T,R>&  appendDifferent(const PmArray<T,R>& array);

	// Removing array-elements.
	//
	PmArray<T,R>&         removeAt    (int index);
	bool                  remove      (const T& value, int start = 0);
	PmArray<T,R>&         removeFirst ();
	PmArray<T,R>&         removeLast  ();
	PmArray<T,R>&         removeAll   ();
	PmArray<T,R>&         removeSubArray (int startIndex, int endIndex);
	...
	PmArray<T,R>&         setLogicalLength(int);
	int                 physicalLength() const;
	PmArray<T,R>&         setPhysicalLength(int);

	// Automatic resizing.
	//
	int                 growLength  () const;
	PmArray<T,R>&         setGrowLength(int);

	// Utility.
	//
	PmArray<T,R>&         reverse     ();
	PmArray<T,R>&         swap        (int i1, int i2);
...
PmArray<T,R>::contains(const T& value, int start) const
{ return this->findFrom(value, start) != -1; }

template <class T, class R> inline int
PmArray<T,R>::length() const
{ return mLogicalLen; }

template <class T, class R> inline bool
PmArray<T,R>::isEmpty() const
{ return mLogicalLen == 0; }

template <class T, class R> inline int
PmArray<T,R>::logicalLength() const
{ return mLogicalLen; }

template <class T, class R> inline int
PmArray<T,R>::physicalLength() const
{ return mPhysicalLen; }

template <class T, class R> inline int
PmArray<T,R>::growLength() const
{ return mGrowLen; }

template <class T, class R> inline const T*
PmArray<T,R>::asArrayPtr() const
{ return mpArray; }

template <class T, class R> inline T*
PmArray<T,R>::asArrayPtr()
{ return mpArray; }

template <class T, class R> inline bool
PmArray<T,R>::isValid(int i) const
{ return i >= 0 && i < mLogicalLen; }

template <class T, class R> inline T&
PmArray<T,R>::operator [] (int i)
{ AC_ARRAY_ASSERT(this->isValid(i)); return mpArray[i]; }

template <class T, class R> inline const T&
PmArray<T,R>::operator [] (int i) const
{ AC_ARRAY_ASSERT(this->isValid(i)); return mpArray[i]; }

template <class T, class R> inline T&
PmArray<T,R>::at(int i)
{ AC_ARRAY_ASSERT(this->isValid(i)); return mpArray[i]; }

template <class T, class R> inline const T&
PmArray<T,R>::at(int i) const
{ AC_ARRAY_ASSERT(this->isValid(i)); return mpArray[i]; }

template <class T, class R> inline PmArray<T,R>&
PmArray<T,R>::setAt(int i, const T& value)
...
template <class T, class R> inline const T&
PmArray<T,R>::last() const
{ AC_ARRAY_ASSERT(!this->isEmpty()); return mpArray[mLogicalLen-1]; }

template <class T, class R> inline int
PmArray<T,R>::append(const T& value)
{ insertAt(mLogicalLen, value); return mLogicalLen-1; }

/// 添加value到数组，如果数组中已经有value，则不添加直接返回该值所在的位置
template <class T, class R> inline int
PmArray<T,R>::appendDifferent(const T& value)
{
	int ret = this->findFrom(value, 0);
	if(ret < 0)
	{
		return append(value);
	}
...
template <class T, class R> PmArray<T,R>&
PmArray<T,R>::appendDifferent(const PmArray<T,R>& otherArray)
{
	int otherLen = otherArray.length();
	for(int i=0; i<otherLen; i++)
	{
		appendDifferent(otherArray[i]);
	}
	return *this;
}
...
template <class T, class R> inline PmArray<T,R>&
PmArray<T,R>::setGrowLength(int glen)
...
PmArray< T, R > ::PmArray(int physicalLength, int growLength)
: mpArray(NULL),
...
//
template <class T, class R>
PmArray<T,R>::PmArray(const PmArray<T,R>& src)
: mpArray(NULL),
...
//
template <class T, class R> PmArray<T,R>&
PmArray<T,R>::operator = (const PmArray<T,R>& src)
{
	if (this != &src) {
...
PmArray<T,R>::operator == (const PmArray<T,R>& cpr) const
{
...
//
template <class T, class R> PmArray<T,R>&
PmArray<T,R>::append(const PmArray<T,R>& otherArray)
{
	int otherLen = otherArray.length();
	if (otherLen == 0) {
		return *this;
...
//
template <class T, class R> bool
PmArray<T,R>::find(const T& value, int& index, int start) const
{
...
PmArray<T,R>::find(const T& value) const
{
	return this->findFrom(value, 0);   // search from the beginning
}

template <class T, class R> int
PmArray<T,R>::findFrom(const T& value, int start) const
{
...
// steps.
template <class T, class R> PmArray<T,R>&
PmArray<T,R>::setLogicalLength(int n)
{
	AC_ARRAY_ASSERT(n >= 0);
	if (n > mPhysicalLen) {
...
//
template <class T, class R> PmArray<T,R>&
PmArray<T,R>::setPhysicalLength(int n)
{
	AC_ARRAY_ASSERT(n >= 0);
...
//
template <class T, class R> bool
PmArray<T,R>::remove(const T& value, int start)
{
...
template <class T> void ReleasePmArray(PmArray<T*>& datas)
{
...
template <class T> void ReleaseCArray(CArray<T*,T*&>& datas)
{
	int nLen = datas.GetSize();
	if(nLen == 0)
	{
		return;
	}
...
template <class T> void ReleaseCArray_1(CArray<T*,T*>& datas)
{
	int nLen = datas.GetSize();
	if(nLen == 0)
	{
...
/**
* @brief   添加sourceDatas中不同的数据到datas，如果其中有相同的数据，则不添加
*
*          
* @note  : 
* @param : 
...
* @date  : [9/12/2013]
*/
template <class T> void AddDifferentArray(const PmArray<T*>& sourceDatas, PmArray<T*>& datas)
{
	int nLen = sourceDatas.length();
	if(nLen == 0)
	{
		return;
	}
