...
class PMProjectDBSERVICE CAccessProject_GraphicsResultV2Table :
	public CPMSLAccessTable
{
public:
	CAccessProject_GraphicsResultV2Table(void);

	virtual ~CAccessProject_GraphicsResultV2Table(void);
	...
	/// 根据comid、handle 删除记录
	BOOL DelResultComIdHandle_Component(const CUIntArray & arrComID,const CStringArray & arrCADHandle);
...
public:
	...
	/**
	 * @brief 根据当前楼层和CADHandle来删除报表中的钢筋量。
	 *
	 *        
	 * @author 郑琪崇
	 * @version v1.0
	 * @date  2016年7月8日   14:53
	 * @note  
	 */
	void DeleteResultInfoHandle( const int iCurFloorId,const CString strHandle );
public:
...
class PMProjectDBSERVICE CProjectGraphicsResultV2Opr
{
public:
	CProjectGraphicsResultV2Opr() ;
	~CProjectGraphicsResultV2Opr() ; 
