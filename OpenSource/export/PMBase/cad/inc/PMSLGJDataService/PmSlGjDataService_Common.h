...
namespace pmdataservice
{
	...
	enum
	{
		...
		eResultType_One = 1 ,	/// 自动计算出来的钢筋结果-----，此结果在重新计算时允许删除
		eResultType_Two	= 2 ,	/// 新增，复制的 钢筋结果 -----，此结果在重新计算时给予保留
	...
	/// zfy add  2015.01.21 为了BIM代码的维护方便，开启如下变量访问
	...
	inline PMTSTRING GetStr( const CUIntArray & ary ) 
	{
		...
		for ( ; i < iSize ; ++i )
		{
			...
			strReturn.append( pmcommon::CStr(value) ) ;
		}
...
const SDbConstName g_dbConstName ; 
