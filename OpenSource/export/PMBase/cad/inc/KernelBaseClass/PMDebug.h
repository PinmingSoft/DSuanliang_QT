...
namespace PMERROR
{
	...
	//获取错误输出的位置
	KERNELBASECLASS_API CString GetErrorFilePath();
	//输出错误
	KERNELBASECLASS_API BOOL PrintInfo(const CString & strErrorMsg,DWORD error,ErrorStyle style);
	KERNELBASECLASS_API BOOL PrintInfo(const CString & strErrorMsg,DWORD error);

	KERNELBASECLASS_API BOOL PrintStringInfo(const std::string & strErrorMsg,DWORD error);

	KERNELBASECLASS_API void SetErrorLogDir(CString strLogDir);

	KERNELBASECLASS_API void SetErrorLogFileName(const CString & strFileName);
}
