...
* @date  : [15/1/2014]
*/
class DATASETDLLEXPORT NDataModelDataSet : public NDataModelBaseDataSet  
{
...
public:
	...
	*********************************************************************/ 
	...
	时    间：[2020.5.18] 
	*********************************************************************/ 
	int Find(void* pBuff,CString sFieldName,eBufferFlag BufferFlag = Primary);
	...
	//删除范围[start,end] 内的所有行，要求end>= start
	...
	*/
	...
	BOOL _ImportXML(VARIANT filename, BSTR xml, BOOL bFromFile, BOOL bAppend, BOOL bAsNew);
	...
	virtual BOOL	SetAsString(int Column, int row ,void* val, int buffLen);
	...
	virtual BOOL	SetAsString(int Column, NRecordID id ,void *val, int buffLen);
...
protected:
	...
	virtual BOOL	SetAsString(NDataField * field, NRecordID id ,void* val, int buffLen);
...
//	virtual eGetResult GetRecord(char * pbuff , eGetMode getmode );
	...
	//外部的行状态标记设置
	inline void SetRowNewExternal(void * pbuff, BOOL isnew);
